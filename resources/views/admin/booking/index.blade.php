			...
			<div class="col-lg-12">
				<div class="card" >
					<div class="card-body">
						...
						<div class="row">
						<div class="col-md-6">
						<input type="text" name="from_date" class="form-control from_date" placeholder="Select starting date" autocomplete="off" />
						</div>
						<div class="col-md-6">
						<input type="text" name="to_date" class="form-control to_date" placeholder="Select ending date" autocomplete="off"/>
						</div>
						</div>
						<div class="row" style="margin-top:20px;margin-left:10px;">
						<div class="col-md-6">
						<a class="filter_apply" href="javascript:void(0);"><span class="badge badge-info">{{trans('admin.Apply')}}</span></a>
							<a href="{{url('admin/export-booking')}}" class="excel"><span class="badge badge-info">Export</span></a>
						</div>
						</div>
...
function ajaxCall(id=0, text='', orderby, order, page=1 , status='',type='') {
	...
	$('.to_date').on('apply.daterangepicker', function(ev, picker) {
		...
		var url = "{{url()->current()}}";
		new_url1 = url.replace( /[\?#].*|$/, "?from_date=" + from_date+"&to_date="+to_date+"&status="+status+"&text="+text+"&approved_by="+approved_by+"&export_type=excel");
		$('.excel').attr('href',new_url1);
