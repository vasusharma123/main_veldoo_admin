    ...
    <div class="userBox mt-3">
        <div class="avatarImg_diver">
            @if (!empty($ride_detail->vehicle->vehicle_image))
                <img src="{{ env('URL_PUBLIC') . '/' . $ride_detail->vehicle->vehicle_image }}" alt="car"
                    class="img-fluid DriverImage ">
            @else
                <img src="{{ asset('company/assets/imgs/sideBarIcon/redcar.png') }}" alt="car"
                    class="img-fluid DriverImage ">
            @endif
        </div>
        <div class="user_name">
