...
class NotificationController extends Controller
{
    ...
    public function last_unseen(Request $request)
    {
        ...
        try {
            $query = Notification::where(['user_id' => $userDetail->id, 'status' => 0]);
            if($userDetail->user_type == 2){
                $query->where(['service_provider_id' => $userDetail->service_provider_id]);
            }
            $last_unseen_notification = $query->whereIn('type',[1,4,10,11,12,13,16])->orderBy('id','desc')->first();
