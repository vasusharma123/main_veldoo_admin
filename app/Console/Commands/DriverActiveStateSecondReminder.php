    ...
    public function handle()
    {
        ...
        if (!empty($overallSettings) && count($overallSettings) > 0) {
            foreach ($overallSettings as $setting) {
                ...
                $unactive_driver_ids = DriverStayActiveNotification::where('last_activity_time', '<=', $alertTime)->where(['is_availability_alert_sent' => 1, 'is_availability_changed' => 1, 'is_logout_alert_sent' => 0, 'service_provider_id' => $setting->service_provider_id])->pluck('driver_id')->toArray();
                if (count($unactive_driver_ids) > 0) {
                    ...
                    foreach ($unactive_driver_ids as $driverid) {
                        $notification_data[] = ['title' => $title, 'description' => $message, 'type' => 13, 'user_id' => $driverid, 'additional_data' => json_encode($additional), 'service_provider_id' => $setting->service_provider_id, 'created_at' => Carbon::now(), 'updated_at' => Carbon::now()];
                    }
