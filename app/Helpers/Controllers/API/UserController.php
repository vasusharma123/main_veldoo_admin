...
print_r($data['results'][0]['geometry']['location']['lng']); */
	...
	public function rideDetail(Request $request)
	{
		...
		try {
			...
			if (isset($request->company_id)) {
				...
				$company_data = DB::table('companies')->select('id as company_id,user_name,name,email,country_code,phone,image,country,state,city,zip,address,status')->where([['id', '=', $request->company_id]])->first();
				if (!empty($company_data)) {
					$ride['company_data'] = $company_data;
				} else {
					$ride['company_data'] = null;
				}
			} else {
				$ride['company_data'] = null;
			}
  ...
  $amount['pool'] = round((2*$amountsetpool),2); */
	...
	public function createRideDriver(Request $request)
	{
		$rules = [
			'pickup_location' => 'required',
			//'drop_location' => 'required',
		...
		$query->where([['user_type', '=', 2], ['availability', '=', 1]])->having('distance', '<', $driver_radius)->orderBy('distance', 'asc')->limit($driverlimit);
		//$query->where('user_type', '=',2)->orderBy('distance','asc');
		$drivers = $query->whereNotNull('device_token')->get()->toArray();
  ...
  curl_close($curl_handle); */
	...
	public function RideList(Request $request)
	{
 		try {
			...
			$userlogintime = $user->updated_at;
			 // print_r($user->is_master);die;
			if (!empty($user)) {
				if ($request->type == 1) {
					if ($user->user_type == 1) {
						//$rides=Ride::where('user_id',$userId)->whereDate('rides.ride_time','>=',Carbon::today())->orderBy('id', 'desc')->with('driver')->paginate($this->limit);
						...
						if ($user->is_master == 1) {
							...
							$ownridesplanned = Ride::where('driver_id', $userId)->where('ride_type', 1)->whereDate('rides.ride_time', '>=', $userlogintime)->where(function ($query) {
								$query->where([['status', '=', 0]])->orWhere([['status', '=', 1]])->orWhere([['status', '=', 2]])->orWhere([['status', '=', 4]]);
							})->orderBy('ride_time', 'desc')->orderBy('status', 'asc')->with('user', 'driver')->paginate($this->limit);
							...
							//	$rides=Ride::where('driver_id',$userId)->whereDate('rides.ride_time','>=',Carbon::today())->orderBy('id', 'desc')->with('user')->paginate($this->limit);
					...
					if ($user->user_type == 1) {
						$rides = Ride::where('user_id', $userId)->whereDate('rides.ride_time', '>=', $userlogintime)->where('status', 3)->orderBy('id', 'desc')->with('driver')->paginate($this->limit);
					} elseif ($user->user_type == 2) {
						if ($user->is_master == 1) {
							if ($request->master_driver == 1) {
								$rides = Ride::where('driver_id', $userId)->whereDate('rides.ride_time', '>=', $userlogintime)->where('status', 3)->orderBy('ride_time', 'desc')->with('user', 'driver')->paginate($this->limit);
							} else {
								$rides = Ride::where('status', 3)->whereDate('rides.ride_time', '>=', $userlogintime)->orderBy('id', 'desc')->with('user', 'driver')->paginate($this->limit);
							}
						} else {
							$rides = Ride::where('driver_id', $userId)->whereDate('rides.ride_time', '>=', $userlogintime)->where('status', 3)->orderBy('ride_time', 'desc')->with('user', 'driver')->paginate($this->limit);
						}
					...
					if ($user->user_type == 1) {
						...
						if ($user->is_master == 1) {
							...
							if ($request->master_driver == 1) {
								...
								$rides = Ride::whereDate('rides.ride_time', '>=', $userlogintime)->orWhere([['status', '=', -1]])->orWhere([['status', '=', -2]])->orWhere([['status', '=', -3]])->orderBy('ride_time', 'desc')->with('user', 'driver')->paginate($this->limit);
			...
			$query->where([['status', '=', -1]])->orWhere([['status', '=', -2]])->orWhere([['status', '=', -3]]);
						...
						if ($user->is_master == 1) {
							if ($request->master_driver == 1) {
								...
								$rides = Ride::whereDate('rides.ride_time', '>=', $userlogintime)->where([['status', '=', 0], ['ride_type', '>', 1]])->whereRaw('FIND_IN_SET(?,driver_id)', [$userId])->orderBy('ride_time', 'desc')->with('user', 'driver')->limit(1)->paginate(1);
							} else {
								$rides = Ride::whereDate('rides.ride_time', '>=', $userlogintime)->where([['status', '=', -4], ['ride_type', '>', 1]])->orderBy('ride_time', 'desc')->with('user', 'driver')->limit(1)->paginate(1);
							}
						} else {
							$rides = Ride::whereDate('rides.ride_time', '>=', $userlogintime)->where([['status', '=', 0], ['ride_type', '>', 1]])->whereRaw('FIND_IN_SET(?,driver_id)', [$userId])->orderBy('ride_time', 'desc')->with('user', 'driver')->limit(1)->paginate(1);
						}
				...
				if (!empty($datarides)) {
					...
					foreach ($datarides as $ridRow) {
						...
						if (isset($ridRow->company_id) && $ridRow->company_id!=null) {
							$company_data = DB::table('companies')->select('id','user_name','name','email','country_code','phone','image','country','state','city','zip','street','status')->where([['id', '=', $ridRow->company_id]])->first();
							if (!empty($company_data)) {
								$datarides[$a]['company_data'] = $company_data;
							} else {
								$datarides[$a]['company_data'] = null;
							}
						} else {
							$datarides[$a]['company_data'] = null;
						}
				...
				if (!empty($rides)) {
					...
					foreach ($rides as $ridRow) {
						...
						if (isset($ridRow->company_id)&& $ridRow->company_id!=null) {
							...
							if (!empty($company_data)) {
								$rides[$b]['company_data'] = $company_data;
							} else {
								$rides[$b]['company_data'] = null;
							}
						} else {
							
							$datarides[$b]['company_data'] = null;
						}
						...
						if ($ridRow['driver']) {
							...
							// $rides[$b]['driver']->car_data = null;
							// $rides[$b]['driver']->avg_rating = null;
						}
						$b++;
					}
				}
	...
	public function driverlistforMaster(Request $request)
	{
		...
		try {
			...
			if (!empty($lat) && !empty($lon)) {
				...
				$query = User::where([['user_type', '=', 2], ['availability', '=', 1]])->orderBy('created_at', 'desc')->limit($driverlimit);
				//$query->where('user_type', '=',2)->orderBy('distance','asc');
				$drivers = $query->get()->toArray();
			}
	...
	public function assigndrivertoRide(Request $request)
	{
		...
		try {
			...
			if ($ride_data->save()) {
				...
				//$driver_data= User::query()->where([['id', '=', $ride['driver_id']]])->first();
				$driver_data = User::select('id', 'first_name', 'last_name', 'image', 'current_lat', 'current_lng', 'country_code', 'phone', 'user_type')->where('id', $request->driver_id)->first();
				...
				$driverdata = User::query()->where([['id', '=', $request->driver_id]])->first();
	...
	public function onlineDrivers(Request $request)
	{
		try {
			...
			$userdatacount = \App\User::where('availability', 1)->where('user_type', 2)->count();


			return response()->json(['success' => true, 'message' => 'get successfully', 'data' => $userdatacount], $this->successCode);
		} catch (\Illuminate\Database\QueryException $exception) {
			$errorCode = $exception->errorInfo[1];
			return response()->json(['message' => $exception->getMessage()], $this->warningCode);
		} catch (\Exception $exception) {
